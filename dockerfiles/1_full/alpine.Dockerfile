...
RUN \
    ...
    gdb && \
    # install minor deps
    pip install -U "asyncpg" "coverage" "flake8" "lxml" "matplotlib" "Pillow" "psutil" "pycryptodome" "pylint" "pytest-cov" "pytest-faulthandler" "ruamel.yaml" "uvloop" "youtube-dl" -q --retries 30 && \
    # remove caches
