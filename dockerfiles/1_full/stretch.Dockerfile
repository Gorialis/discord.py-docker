...
RUN apt-get update && \
    ...
    > /dev/null && \
    # install minor deps
    pip install -U "asyncpg" "coverage" "flake8" "lxml" "matplotlib" "git+https://github.com/numpy/numpy@master#egg=numpy" "Pillow" "psutil" "pycryptodome" "pylint" "pytest-cov" "pytest-faulthandler" "ruamel.yaml" "uvloop" "youtube-dl" -q --retries 30 && \
    # remove caches
